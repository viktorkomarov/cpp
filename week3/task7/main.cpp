...
class Person {
public:
  Person(const std::string& name, const std::string& surname, const int y):birthday(y){
      name_by_year[y] = name;
      surname_by_year[y] = surname;
  }
...
private:
  ...
  std::string get_value(int year, const std::map<int, std::string>& values) const {
      std::string res;
  ...
  std::string build_history(const std::vector<std::string>& val) const {
      if (val.empty()) return "";
  ...
  std::vector<std::string> get_values_before(int year, const std::map<int, std::string>& values) const {
